...
import { calculatePlanMetrics } from '@/lib/plan-math';
...
export function summarizePlan(state: Store = useMediaPlanStore.getState()) {
  const result = calculatePlanMetrics(state.channels, state.totalBudget, state.globalMultipliers);
  const b = result.blendedMetrics;
  return {
    totalBudget: state.totalBudget,
    channels: result.channelsWithMetrics.map(c => ({
      id: c.id,
      name: c.name,
      model: c.buyingModel,
      spend: round(c.metrics.spend),
      sharePct: round(c.allocationPct, 1),
    ...
    blended: {
      ...
      leads: round(b.totalRegistrations),
      conversions: round(b.totalConversions, 1),
      costPerLead: round(b.blendedCpl),
