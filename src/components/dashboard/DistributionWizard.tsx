...
function getStrategyIcon(strategy: DistributionStrategy) {
  switch (strategy) {
    ...
    case 'affiliate_dominant':
      return <Users className="h-4 w-4 text-purple-500" />;
    case 'influencer_dominant':
...
function getBarWidths(strategy: DistributionStrategy): [string, string, string, string] {
  switch (strategy) {
    ...
    case 'hybrid_growth':
      return ['15%', '35%', '35%', '15%'];
    case 'seo_foundation':
      return ['60%', '20%', '10%', '10%']; // Heavy Blue (SEO/Content)
    case 'conversion_max':
      return ['10%', '10%', '20%', '60%']; // Heavy Orange (Retargeting/Push)
    case 'programmatic_blitz':
      return ['20%', '10%', '20%', '50%']; // Mix
    case 'retention_ltv':
      return ['10%', '50%', '10%', '30%']; // Mix
    default:
      return ['25%', '25%', '25%', '25%'];
  }
}
      ...
      {showTrigger && (
        <DialogTrigger asChild>
          <Button
            ...
            size="sm"
            className={cn(
              'h-9 px-3 text-sm gap-2 bg-sidebar-accent border-sidebar-border text-sidebar-foreground hover:bg-sidebar-primary hover:text-sidebar-primary-foreground transition-colors',
              triggerClassName
            )}
          >
      ...
      <DialogContent className="sm:max-w-[800px]">
        ...
        <div className="grid gap-6 py-4 grid-cols-[1.5fr_1fr]">
          <div className="grid grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2">
            {(Object.keys(DISTRIBUTION_CONFIG.strategies) as DistributionStrategy[]).map(
              (strategy) => {
